---
name: AI Coach
description: Elite-only streaming AI chat coach using GPT-5 via Lovable AI Gateway, with 7d memory + proactive morning nudges
type: feature
---
...
- **Memory**: system prompt now injects 7-day stats summary (workouts, sleep, hydration, perfect days, yesterday detail) + last briefing's `key_insights`
...
- Per Elite user with checkin yesterday: AI generates 1-2 sentence focus via `emit_nudge` tool call (gemini-3-flash)
- One nudge max per user per day (skip if any nudge today)
- `CoachNudgeCard` on Index.tsx (Elite, unseen): gold card with sparkles icon, taps → `/coach`
