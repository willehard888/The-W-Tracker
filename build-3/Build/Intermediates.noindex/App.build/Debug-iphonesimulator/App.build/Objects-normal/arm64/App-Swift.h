...
/// <code>AVCaptureMetadataOutput</code>. UPC-A arrives as EAN-13 with a leading 0; the
/// GTIN inside a Code 128 / Digital Link payload is extracted JS-side.
/// Replaces <code>@capacitor-mlkit/barcode-scanning</code>: the GoogleMLKit pods inject
/// <code>EXCLUDED_ARCHS[sdk=iphonesimulator*] = arm64</code>, which makes every simulator
...
/// Like <code>HealthNight</code>, this is compiled DIRECTLY INTO THE APP TARGET and
/// registered in code from <code>MainViewController.capacitorDidLoad()</code> — see that
/// file and HealthNight.swift for why (no stand-alone pod, and <code>cap copy</code>
/// regenerates <code>packageClassList</code>).
SWIFT_CLASS_NAMED("BarcodeScan")
@interface BarcodeScan : CAPPlugin <CAPBridgedPlugin>
...
- (void)openSettings:(CAPPluginCall * _Nonnull)call;
- (void)scan:(CAPPluginCall * _Nonnull)call;
...
/// <code>MainViewController.capacitorDidLoad()</code> (via <code>bridge.registerPluginInstance</code>),
/// NOT as a separate CocoaPods pod and NOT via <code>capacitor.config.json</code>.
/// Rationale:
/// <ol>
///   <li>
///     Under Xcode 26.5 a stand-alone Swift pod cannot resolve <code>import Capacitor</code>
...
///   </li>
///   <li>
///     CI runs <code>npx cap copy ios</code>, which regenerates <code>packageClassList</code> from npm
///     plugins every build — so config-based auto-registration would silently drop
///     a local class. Explicit <code>registerPluginInstance</code> is immune to that.
///   </li>
/// </ol>
SWIFT_CLASS_NAMED("HealthNight")
@interface HealthNight : CAPPlugin <CAPBridgedPlugin>
@property (nonatomic, readonly, copy) NSString * _Nonnull identifier;
@property (nonatomic, readonly, copy) NSString * _Nonnull jsName;
@property (nonatomic, readonly, copy) NSArray<CAPPluginMethod *> * _Nonnull pluginMethods;
- (void)requestAuthorization:(CAPPluginCall * _Nonnull)call;
- (void)queryNight:(CAPPluginCall * _Nonnull)call;
- (void)requestMealWriteAuthorization:(CAPPluginCall * _Nonnull)call;
- (void)writeMeal:(CAPPluginCall * _Nonnull)call;
- (void)deleteMeal:(CAPPluginCall * _Nonnull)call;
- (void)requestWorkoutWriteAuthorization:(CAPPluginCall * _Nonnull)call;
- (void)writeWorkout:(CAPPluginCall * _Nonnull)call;
- (nonnull instancetype)initWithBridge:(id <CAPBridgeProtocol> _Nonnull)bridge pluginId:(NSString * _Nonnull)pluginId pluginName:(NSString * _Nonnull)pluginName OBJC_DESIGNATED_INITIALIZER SWIFT_DEPRECATED_MSG("This initializer is deprecated and is not suggested for use. Any data set through this init method will be overridden when it is loaded on the bridge.");
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// Registers the App-target-local plugins (<code>HealthNight</code>, <code>BarcodeScan</code>)
/// explicitly. This is the Capacitor-documented way to register a plugin that
/// is NOT an npm package: <code>capacitorDidLoad()</code> runs after the bridge is
/// created, and <code>registerPluginInstance</code> bypasses the <code>autoRegisterPlugins</code>
/// config path.
/// Why not <code>capacitor.config.json</code> → <code>packageClassList</code>? Because CI runs
/// <code>npx cap copy ios</code>, which REGENERATES that list from installed npm plugins on
...
@interface MainViewController : CAPBridgeViewController
