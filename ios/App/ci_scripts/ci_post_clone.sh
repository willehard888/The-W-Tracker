...
# ── Verify Package.resolved exists (required by Xcode Cloud) ──
RESOLVED_FILE="ios/App/App.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved"
if [[ -f "$RESOLVED_FILE" ]]; then
  echo "✅ Package.resolved found — Xcode Cloud will use it"
else
  echo "❌ Package.resolved missing — Xcode Cloud requires it in source control"
  exit 1
fi
