...
# ---------------------------------------------------------------------------
# SPM Package.resolved regeneration
# ---------------------------------------------------------------------------
# `cap sync` (above) can mutate ios/App/CapApp-SPM/Package.swift whenever
# node_modules layout drifts (new plugin packages added / versions bumped).
# That makes the committed Package.resolved look "out of date" to the main
# xcodebuild step Xcode Cloud runs next — which fails with:
#   "an out-of-date resolved file was detected ... which is not allowed
#    when automatic dependency resolution is disabled".
#
# Build numbers 773, 775, 776 all died at this step. Deleting the committed
# file here lets Xcode Cloud's own resolver write a fresh one against the
# *just-synced* Package.swift before -onlyUsePackageVersionsFromResolvedFile
# can complain.
RESOLVED="$IOS_APP_DIR/App.xcworkspace/xcshareddata/swiftpm/Package.resolved"
if [[ -f "$RESOLVED" ]]; then
  echo "🧹 Removing committed Package.resolved (will be regenerated by Xcode Cloud)..."
  rm -f "$RESOLVED"
fi

# Pre-resolve so the resolved file exists *before* xcodebuild's main step
# runs with -onlyUsePackageVersionsFromResolvedFile. Non-fatal: if this
# fails the main build still owns the final attempt.
if command -v xcodebuild &>/dev/null; then
  echo "📦 Pre-resolving SPM packages against freshly synced Package.swift..."
  xcodebuild -resolvePackageDependencies \
    -workspace "$IOS_APP_DIR/App.xcworkspace" \
    -scheme App 2>&1 | tail -5 \
    || echo "⚠️ Pre-resolve had warnings — letting main xcodebuild step retry"
fi
