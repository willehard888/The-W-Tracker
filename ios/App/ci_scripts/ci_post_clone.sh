...
# ---------------------------------------------------------------------------
echo "🔄 Copying web bundle into iOS project (cap copy, not sync)..."
# IMPORTANT: use `cap copy` not `cap sync`. `sync` rewrites
# ios/App/CapApp-SPM/Package.swift to whatever node_modules contains —
# which then makes the committed Package.resolved look "out of date" to
# Xcode Cloud's `xcodebuild -onlyUsePackageVersionsFromResolvedFile`
# step, killing builds 773, 775, 776, and 777. `cap copy` only copies
# the web bundle (dist/ → ios/App/App/public) and refreshes
# capacitor.config.json. It does NOT touch Package.swift, so the
# committed Package.swift + Package.resolved pair stays internally
# consistent.
#
# If you add a new Capacitor plugin you MUST run `npx cap sync ios`
