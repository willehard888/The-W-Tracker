...
-- Generated: 2026-05-20T06:22:16.771869+00:00
-- Source project: zjdljojkgrpgxurugixf
--
-- IMPORTANT:
-- - public schema below is a real pg_dump from the direct sandbox DB role.
-- - full auth schema pg_dump is blocked for this role (permission denied on auth schema locks).
-- - auth.users INSERT statements below were obtained through the internal read-only database tool.
-- - For a production-grade auth migration, prefer a real `pg_dump --schema=auth` run by Lovable support/admin.


-- ============================================================
-- A. Direct auth pg_dump blocker proof
-- ============================================================
...
-- ============================================================
-- B. Approximate CREATE TABLE statements from information_schema
-- ============================================================
CREATE TABLE auth.audit_log_entries (instance_id uuid, id uuid NOT NULL, payload json, created_at timestamptz, ip_address varchar NOT NULL);
CREATE TABLE auth.flow_state (id uuid NOT NULL, user_id uuid, auth_code text, code_challenge_method code_challenge_method, code_challenge text, provider_type text NOT NULL, provider_access_token text, provider_refresh_token text, created_at timestamptz, updated_at timestamptz, authentication_method text NOT NULL, auth_code_issued_at timestamptz, invite_token text, referrer text, oauth_client_state_id uuid, linking_target_id uuid, email_optional bool NOT NULL);
CREATE TABLE auth.identities (provider_id text NOT NULL, user_id uuid NOT NULL, identity_data jsonb NOT NULL, provider text NOT NULL, last_sign_in_at timestamptz, created_at timestamptz, updated_at timestamptz, email text, id uuid NOT NULL);
CREATE TABLE auth.instances (id uuid NOT NULL, uuid uuid, raw_base_config text, created_at timestamptz, updated_at timestamptz);
CREATE TABLE auth.mfa_amr_claims (session_id uuid NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, authentication_method text NOT NULL, id uuid NOT NULL);
CREATE TABLE auth.mfa_challenges (id uuid NOT NULL, factor_id uuid NOT NULL, created_at timestamptz NOT NULL, verified_at timestamptz, ip_address inet NOT NULL, otp_code text, web_authn_session_data jsonb);
CREATE TABLE auth.mfa_factors (id uuid NOT NULL, user_id uuid NOT NULL, friendly_name text, factor_type factor_type NOT NULL, status factor_status NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, secret text, phone text, last_challenged_at timestamptz, web_authn_credential jsonb, web_authn_aaguid uuid, last_webauthn_challenge_data jsonb);
CREATE TABLE auth.one_time_tokens (id uuid NOT NULL, user_id uuid NOT NULL, token_type one_time_token_type NOT NULL, token_hash text NOT NULL, relates_to text NOT NULL, created_at timestamp NOT NULL, updated_at timestamp NOT NULL);
CREATE TABLE auth.refresh_tokens (instance_id uuid, id int8 NOT NULL, token varchar, user_id varchar, revoked bool, created_at timestamptz, updated_at timestamptz, parent varchar, session_id uuid);
CREATE TABLE auth.saml_providers (id uuid NOT NULL, sso_provider_id uuid NOT NULL, entity_id text NOT NULL, metadata_xml text NOT NULL, metadata_url text, attribute_mapping jsonb, created_at timestamptz, updated_at timestamptz, name_id_format text);
CREATE TABLE auth.saml_relay_states (id uuid NOT NULL, sso_provider_id uuid NOT NULL, request_id text NOT NULL, for_email text, redirect_to text, created_at timestamptz, updated_at timestamptz, flow_state_id uuid);
CREATE TABLE auth.schema_migrations (version varchar NOT NULL);
CREATE TABLE auth.sessions (id uuid NOT NULL, user_id uuid NOT NULL, created_at timestamptz, updated_at timestamptz, factor_id uuid, aal aal_level, not_after timestamptz, refreshed_at timestamp, user_agent text, ip inet, tag text, oauth_client_id uuid, refresh_token_hmac_key text, refresh_token_counter int8, scopes text);
CREATE TABLE auth.sso_domains (id uuid NOT NULL, sso_provider_id uuid NOT NULL, domain text NOT NULL, created_at timestamptz, updated_at timestamptz);
CREATE TABLE auth.sso_providers (id uuid NOT NULL, resource_id text, created_at timestamptz, updated_at timestamptz, disabled bool);
CREATE TABLE auth.users (instance_id uuid, id uuid NOT NULL, aud varchar, role varchar, email varchar, encrypted_password varchar, email_confirmed_at timestamptz, invited_at timestamptz, confirmation_token varchar, confirmation_sent_at timestamptz, recovery_token varchar, recovery_sent_at timestamptz, email_change_token_new varchar, email_change varchar, email_change_sent_at timestamptz, last_sign_in_at timestamptz, raw_app_meta_data jsonb, raw_user_meta_data jsonb, is_super_admin bool, created_at timestamptz, updated_at timestamptz, phone text, phone_confirmed_at timestamptz, phone_change text, phone_change_token varchar, phone_change_sent_at timestamptz, confirmed_at timestamptz, email_change_token_current varchar, email_change_confirm_status int2, banned_until timestamptz, reauthentication_token varchar, reauthentication_sent_at timestamptz, is_sso_user bool NOT NULL, deleted_at timestamptz, is_anonymous bool NOT NULL);
CREATE TABLE public.badges (id uuid NOT NULL, name text NOT NULL, description text, icon text NOT NULL, rarity badge_rarity NOT NULL, category text NOT NULL, requirement_type text, requirement_value int4, created_at timestamptz NOT NULL);
CREATE TABLE public.battle_votes (id uuid NOT NULL, battle_id uuid NOT NULL, voter_id uuid NOT NULL, voted_for uuid NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.battles (id uuid NOT NULL, challenger_id uuid NOT NULL, opponent_id uuid NOT NULL, status battle_status NOT NULL, winner_id uuid, battle_type text NOT NULL, duration_days int4 NOT NULL, started_at timestamptz, ended_at timestamptz, created_at timestamptz NOT NULL, challenger_score int4 NOT NULL, opponent_score int4 NOT NULL, challenger_proof_url text, opponent_proof_url text, challenger_start_xp int4 NOT NULL, opponent_start_xp int4 NOT NULL);
CREATE TABLE public.coach_athlete_profile (user_id uuid NOT NULL, age int4, sex text, height_cm numeric, weight_kg numeric, body_fat_pct numeric, primary_goal text, secondary_goal text, target_horizon_weeks int4, timezone text NOT NULL, wake_time time NOT NULL, sleep_time time NOT NULL, training_days_pref _int4 NOT NULL, busy_blocks jsonb NOT NULL, injuries _text NOT NULL, dietary _text NOT NULL, equipment _text NOT NULL, no_go_protocols _text NOT NULL, language_pref text NOT NULL, tone_pref text NOT NULL, preferred_session_length_min int4 NOT NULL, i_am text, onboarded bool NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.coach_chat_memory (id uuid NOT NULL, user_id uuid NOT NULL, fact text NOT NULL, source text NOT NULL, confidence numeric NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.coach_daily_briefs (id uuid NOT NULL, user_id uuid NOT NULL, brief_date date NOT NULL, payload jsonb NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.coach_daily_plans (id uuid NOT NULL, user_id uuid NOT NULL, plan_date date NOT NULL, readiness_score int4 NOT NULL, readiness_breakdown jsonb NOT NULL, adjustment text NOT NULL, headline text, missions jsonb NOT NULL, generated_with text NOT NULL, generated_at timestamptz NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, rationale text, framework_version text NOT NULL);
CREATE TABLE public.coach_goals (id uuid NOT NULL, user_id uuid NOT NULL, title text NOT NULL, metric text NOT NULL, unit text NOT NULL, baseline_value numeric, current_value numeric, target_value numeric NOT NULL, deadline date, weekly_milestone numeric, status text NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.coach_mission_logs (id uuid NOT NULL, user_id uuid NOT NULL, daily_plan_id uuid NOT NULL, mission_id text NOT NULL, xp_awarded int4 NOT NULL, completed_at timestamptz NOT NULL);
CREATE TABLE public.coach_nudges (id uuid NOT NULL, user_id uuid NOT NULL, content text NOT NULL, headline text, created_at timestamptz NOT NULL, seen_at timestamptz);
CREATE TABLE public.coach_performance_snapshots (id uuid NOT NULL, user_id uuid NOT NULL, snapshot_date date NOT NULL, performance_score int4 NOT NULL, components jsonb NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.coach_preference_signals (id uuid NOT NULL, user_id uuid NOT NULL, signal_type text NOT NULL, protocol_id text, value text, metadata jsonb NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.coach_program_logs (id uuid NOT NULL, user_id uuid NOT NULL, program_id uuid NOT NULL, week int4 NOT NULL, day_index int4 NOT NULL, completed bool NOT NULL, perceived_rpe int4, notes text, logged_at timestamptz NOT NULL);
CREATE TABLE public.coach_programs (id uuid NOT NULL, user_id uuid NOT NULL, status text NOT NULL, goal text NOT NULL, experience text NOT NULL, days_per_week int4 NOT NULL, equipment text, body_focus _text NOT NULL, constraints text, weeks int4 NOT NULL, plan_json jsonb NOT NULL, ai_summary text, generated_with text NOT NULL, started_on date NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.coach_reflections (id uuid NOT NULL, user_id uuid NOT NULL, reflection_date date NOT NULL, energy_1to5 int4 NOT NULL, rpe_1to10 int4, sleep_quality_1to5 int4, mood_1to5 int4, win text, friction text, created_at timestamptz NOT NULL);
CREATE TABLE public.coach_weekly_reviews (id uuid NOT NULL, user_id uuid NOT NULL, week_starts_on date NOT NULL, performance_score int4 NOT NULL, driver_of_week text, wins jsonb NOT NULL, frictions jsonb NOT NULL, next_week_focus text, program_tweak text, generated_with text NOT NULL, created_at timestamptz NOT NULL, seen_at timestamptz);
CREATE TABLE public.content_moderations (id uuid NOT NULL, content_type text NOT NULL, content_id uuid, image_url text, text_content text, is_safe bool NOT NULL, categories _text NOT NULL, confidence numeric NOT NULL, reason text, action text NOT NULL, model text NOT NULL, created_at timestamptz NOT NULL, severity text, cache_hit bool NOT NULL, latency_ms int4);
CREATE TABLE public.daily_checkins (id uuid NOT NULL, user_id uuid NOT NULL, sleep_hours numeric NOT NULL, workout bool NOT NULL, extra_workout bool NOT NULL, cold_shower bool NOT NULL, healthy_food bool NOT NULL, protein_intake bool NOT NULL, meditation_morning bool NOT NULL, meditation_evening bool NOT NULL, hydration_liters numeric NOT NULL, no_phone_morning bool NOT NULL, no_phone_evening bool NOT NULL, proof_photo_url text, xp_earned int4 NOT NULL, checked_in_at timestamptz NOT NULL, created_at timestamptz NOT NULL, reading bool NOT NULL, journal_entry text);
CREATE TABLE public.direct_messages (id uuid NOT NULL, sender_id uuid NOT NULL, receiver_id uuid NOT NULL, content text NOT NULL, read bool NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.feed_comments (id uuid NOT NULL, post_id uuid NOT NULL, user_id uuid NOT NULL, content text NOT NULL, created_at timestamptz NOT NULL, parent_id uuid, updated_at timestamptz NOT NULL);
CREATE TABLE public.feed_posts (id uuid NOT NULL, user_id uuid NOT NULL, content text, image_url text, likes_count int4 NOT NULL, comments_count int4 NOT NULL, reported bool NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, video_url text, kudos_count int4 NOT NULL);
CREATE TABLE public.feed_reactions (id uuid NOT NULL, post_id uuid NOT NULL, user_id uuid NOT NULL, reaction_type text NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.friendships (id uuid NOT NULL, requester_id uuid NOT NULL, addressee_id uuid NOT NULL, status friendship_status NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.kudos (id uuid NOT NULL, giver_id uuid NOT NULL, post_id uuid NOT NULL, receiver_id uuid NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.leaderboard_champions (id uuid NOT NULL, season_id uuid NOT NULL, user_id uuid NOT NULL, username_snapshot text, season_points int4 NOT NULL, reward_type text NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.leaderboard_season_baselines (id uuid NOT NULL, season_id uuid NOT NULL, user_id uuid NOT NULL, baseline_xp int4 NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.leaderboard_seasons (id uuid NOT NULL, name text NOT NULL, starts_at timestamptz NOT NULL, ends_at timestamptz NOT NULL, status leaderboard_season_status NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.legend_invites (id uuid NOT NULL, code text NOT NULL, created_by uuid NOT NULL, note text, expires_at timestamptz, used_by uuid, used_at timestamptz, created_at timestamptz NOT NULL);
CREATE TABLE public.moderation_cache (image_hash text NOT NULL, action text NOT NULL, categories _text NOT NULL, confidence numeric NOT NULL, severity text, reason text, created_at timestamptz NOT NULL);
CREATE TABLE public.moderation_queue (id uuid NOT NULL, content_type text NOT NULL, content_id uuid, image_url text, text_content text, user_id uuid NOT NULL, ai_action text NOT NULL, ai_confidence numeric NOT NULL, ai_categories _text NOT NULL, ai_reason text, severity text, status text NOT NULL, reviewed_by uuid, reviewed_at timestamptz, created_at timestamptz NOT NULL);
CREATE TABLE public.profiles (id uuid NOT NULL, user_id uuid NOT NULL, username text NOT NULL, display_name text, avatar_url text, xp int4 NOT NULL, level int4 NOT NULL, streak int4 NOT NULL, longest_streak int4 NOT NULL, status_tier status_tier NOT NULL, is_elite bool NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, referral_code text, referred_by uuid, referral_count int4 NOT NULL, featured_badge_id uuid, rank_score numeric NOT NULL, trust_multiplier numeric NOT NULL, rank_score_updated_at timestamptz NOT NULL, trial_started_at timestamptz NOT NULL, last_rank_snapshot jsonb, membership_credits_until timestamptz, referral_milestones_hit jsonb NOT NULL, is_apex_subscriber bool NOT NULL, apex_subscription_started_at timestamptz, apex_credits_until timestamptz, legend_pinned bool NOT NULL, is_premium bool NOT NULL);
CREATE TABLE public.push_tokens (id uuid NOT NULL, user_id uuid NOT NULL, token text NOT NULL, platform text NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.referrals (id uuid NOT NULL, referrer_id uuid NOT NULL, referred_id uuid NOT NULL, rewarded bool NOT NULL, created_at timestamptz NOT NULL, converted bool NOT NULL, converted_at timestamptz);
CREATE TABLE public.reports (id uuid NOT NULL, reporter_id uuid NOT NULL, post_id uuid, reason text NOT NULL, resolved bool NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.tribe_battles (id uuid NOT NULL, challenger_tribe_id uuid NOT NULL, opponent_tribe_id uuid NOT NULL, challenger_owner_id uuid NOT NULL, opponent_owner_id uuid NOT NULL, status tribe_battle_status NOT NULL, duration_days int4 NOT NULL, started_at timestamptz, ended_at timestamptz, challenger_score int4 NOT NULL, opponent_score int4 NOT NULL, winner_tribe_id uuid, created_at timestamptz NOT NULL);
CREATE TABLE public.tribe_invites (id uuid NOT NULL, tribe_id uuid NOT NULL, inviter_id uuid NOT NULL, invitee_id uuid NOT NULL, status text NOT NULL, created_at timestamptz NOT NULL, responded_at timestamptz);
CREATE TABLE public.tribe_members (id uuid NOT NULL, tribe_id uuid NOT NULL, user_id uuid NOT NULL, role text NOT NULL, status text NOT NULL, joined_at timestamptz NOT NULL);
CREATE TABLE public.tribe_post_comments (id uuid NOT NULL, post_id uuid NOT NULL, user_id uuid NOT NULL, parent_id uuid, content text NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.tribe_post_kudos (id uuid NOT NULL, post_id uuid NOT NULL, giver_id uuid NOT NULL, receiver_id uuid NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.tribe_post_reactions (id uuid NOT NULL, post_id uuid NOT NULL, user_id uuid NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.tribe_post_reports (id uuid NOT NULL, post_id uuid NOT NULL, reporter_id uuid NOT NULL, reason text NOT NULL, resolved bool NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.tribe_posts (id uuid NOT NULL, tribe_id uuid NOT NULL, user_id uuid NOT NULL, content text, image_url text, likes_count int4 NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, video_url text, comments_count int4 NOT NULL, kudos_count int4 NOT NULL, reported bool NOT NULL);
CREATE TABLE public.tribes (id uuid NOT NULL, owner_id uuid NOT NULL, name text NOT NULL, slug text NOT NULL, description text, cover_url text, visibility text NOT NULL, member_count int4 NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, is_paused bool NOT NULL, paused_at timestamptz, paused_reason text);
CREATE TABLE public.user_badges (id uuid NOT NULL, user_id uuid NOT NULL, badge_id uuid NOT NULL, earned_at timestamptz NOT NULL);
CREATE TABLE public.user_habit_logs (id uuid NOT NULL, habit_id uuid NOT NULL, user_id uuid NOT NULL, logged_on date NOT NULL, xp_awarded int4 NOT NULL, created_at timestamptz NOT NULL);
CREATE TABLE public.user_habits (id uuid NOT NULL, user_id uuid NOT NULL, protocol_id text NOT NULL, added_at timestamptz NOT NULL, archived_at timestamptz, current_streak int4 NOT NULL, best_streak int4 NOT NULL, level int4 NOT NULL, last_logged_on date, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL);
CREATE TABLE public.user_roles (id uuid NOT NULL, user_id uuid NOT NULL, role app_role NOT NULL);
CREATE TABLE public.vault_articles (id uuid NOT NULL, category_id text NOT NULL, slug text NOT NULL, title text NOT NULL, subtitle text, summary text NOT NULL, evidence_tier text NOT NULL, read_time_min int4 NOT NULL, protocol jsonb NOT NULL, benefits _text NOT NULL, risks _text NOT NULL, body_md text NOT NULL, references_json jsonb NOT NULL, display_order int4 NOT NULL, published_at timestamptz NOT NULL, created_at timestamptz NOT NULL, updated_at timestamptz NOT NULL, lesson_number int4, why_it_matters text, try_today _text NOT NULL, key_takeaways _text NOT NULL, quiz jsonb NOT NULL, course_role text NOT NULL);
CREATE TABLE public.vault_lesson_progress (id uuid NOT NULL, user_id uuid NOT NULL, article_id uuid NOT NULL, completed_at timestamptz NOT NULL, quiz_score int4);
CREATE TABLE public.weekly_briefings (id uuid NOT NULL, user_id uuid NOT NULL, week_start date NOT NULL, week_end date NOT NULL, headline text NOT NULL, summary_md text NOT NULL, key_insights jsonb NOT NULL, next_week_protocol jsonb NOT NULL, stats_snapshot jsonb NOT NULL, generated_at timestamptz NOT NULL, viewed_at timestamptz);

-- ============================================================
-- C. auth.users INSERT statements with encrypted_password hashes
-- ============================================================
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('01a63c98-3dcd-4666-9be1-182d11c3e066', 'rasmus.willehard@gmail.com', '$2a$10$Vk/dTZ2Rvo6cSJe0cr45jOXOfZTxWU7ttyCXpl5Lep02kW/4ITDmG', '2026-03-22 09:46:52.419451+00', '2026-03-22 09:46:52.376104+00', '{"provider": "email", "providers": ["email", "google"]}'::jsonb, '{"iss": "https://accounts.google.com", "sub": "115441800927733729493", "name": "Rasmus Petterson", "email": "rasmus.willehard@gmail.com", "picture": "https://lh3.googleusercontent.com/a/ACg8ocKNEy6Fxp-zLJkEoj8Rxp75ZsIBDN1AfoZ9lqizB5f9q8z6=s96-c", "username": "willehard", "full_name": "Rasmus Petterson", "avatar_url": "https://lh3.googleusercontent.com/a/ACg8ocKNEy6Fxp-zLJkEoj8Rxp75ZsIBDN1AfoZ9lqizB5f9q8z6=s96-c", "provider_id": "115441800927733729493", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('63752d9b-c1ce-498f-9994-d38c462c3c6b', 'rasmus.willehard@gmail.fi', '$2a$10$Kvl8ePHcYYteA0jE5EhuiORlJyI0XWO1SFbW3I1uA4ATdaYij8qOS', '2026-03-22 10:16:29.812962+00', '2026-03-22 10:16:29.78544+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "63752d9b-c1ce-498f-9994-d38c462c3c6b", "email": "rasmus.willehard@gmail.fi", "username": "moneymogger888", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('9cbdb328-fbc4-4a00-b5f6-f093a5b77827', 'rasmus.willehard@gmail.co', '$2a$10$XSLa3hwHgZQ0ClOC2NZEKeXiBv9kIhbF8jvIYmlmKSlOi/ybB0ttm', '2026-03-22 10:43:34.998487+00', '2026-03-22 10:43:34.843688+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "9cbdb328-fbc4-4a00-b5f6-f093a5b77827", "email": "rasmus.willehard@gmail.co", "username": "kingmaxing", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('006339bd-b83a-443a-ade5-2c2efc6d8efd', 'rosa.petterson1@gmail.com', '$2a$10$vtVcZc4CA7gj1Q6WrCzQn.IsrSQYmfKUMwzJsY2KBCCTEoz3TLARC', '2026-03-22 12:32:31.988772+00', '2026-03-22 12:32:31.890267+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "006339bd-b83a-443a-ade5-2c2efc6d8efd", "email": "rosa.petterson1@gmail.com", "username": "rosa11", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('a46ff5cb-c0fe-4ceb-9743-7a68e7a48f6d', 'moi@moi', '$2a$10$zgN6CrWSnI5/.R7qcWmTIecftLucAZFb.DIARCnS92R64AFMAkeXy', '2026-03-22 14:00:42.870299+00', '2026-03-22 14:00:42.796511+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "a46ff5cb-c0fe-4ceb-9743-7a68e7a48f6d", "email": "moi@moi", "username": "elitesensei", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('d95f5868-c4f7-4403-ad85-f2a35797f03b', 'mijjujj@njnnjn.com', '$2a$10$Gqdk4dbVHjTjxgcjhzo85.vKZjqaB/Oeb2HRGKzhF1/JKchfOKieO', '2026-03-22 16:40:14.536595+00', '2026-03-22 16:40:14.502061+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "d95f5868-c4f7-4403-ad85-f2a35797f03b", "email": "mijjujj@njnnjn.com", "username": "wgroup", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('779580c8-93a0-43d2-9022-49a57c0dadc5', 'moimoi@hmiu', '$2a$10$rwBK1KM7QxgTaVUUoixV6OoOdZ9qYj0Ial1LSh9o3b/wnnLBYHYOq', '2026-03-22 17:11:18.327015+00', '2026-03-22 17:11:18.289456+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "779580c8-93a0-43d2-9022-49a57c0dadc5", "email": "moimoi@hmiu", "username": "sisu", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('d7b989be-006f-4e21-aa39-e21642f56d8a', 'iipo.kajalaa@gmail.com', '$2a$10$GxElXgOJDcqlXRn76zE0SOgw/DgzdZpmJj2NNIzDhvix99tunZ8UC', '2026-03-23 07:48:26.109859+00', '2026-03-23 07:48:26.031887+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "d7b989be-006f-4e21-aa39-e21642f56d8a", "email": "iipo.kajalaa@gmail.com", "username": "relentlessrise", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('8a731910-5b1f-4acc-8f27-9d6915054c42', 'rasmus.willehard@gm', '$2a$10$eWIztIuAwkEKXVFA02/nluzaYg2FW/KEZIJUaiPi0Y0CS3KupOHxC', '2026-03-24 10:03:27.578711+00', '2026-03-24 10:03:27.478805+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "8a731910-5b1f-4acc-8f27-9d6915054c42", "email": "rasmus.willehard@gm", "username": "mogger", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('e7d58531-5700-4188-8551-6ef815017aa5', 'mokok@loko', '$2a$10$dk7wJhYOqesTrpOmwYNqL.q/FiAKEwxmPjxDHdDHU/Lcm801V8x4i', '2026-03-24 14:46:33.697906+00', '2026-03-24 14:46:33.656457+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "e7d58531-5700-4188-8551-6ef815017aa5", "email": "mokok@loko", "username": "moimoi", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('2e68a67b-a09b-4e40-bd21-58d1ee1f98c8', 'rasmus.willehard@gmail.commm', '$2a$10$xcs6OI8cCY8SLrgdeCwBt.Z/aJ3imsSUjfhSxKO/m5l1N1McG/lfe', '2026-03-25 07:10:46.407833+00', '2026-03-25 07:10:46.345504+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "2e68a67b-a09b-4e40-bd21-58d1ee1f98c8", "email": "rasmus.willehard@gmail.commm", "username": "mogmaxing", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('f57f89d5-f091-4c8f-a4cf-0702da7bb4c6', 'rasmus.willehard@gmail.commmmm', '$2a$10$I/vLTE7szVYLKglaEMHgh.bN5h/cMwcT.jjm2VIpKlOlTQ.0TeSfy', '2026-03-25 07:13:38.352478+00', '2026-03-25 07:13:38.30286+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "f57f89d5-f091-4c8f-a4cf-0702da7bb4c6", "email": "rasmus.willehard@gmail.commmmm", "username": "mogging", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('5a8e9c0b-545e-4b2b-8058-895831c9dec8', 'rasmus.willehard@gmail.comnbhg', '$2a$10$iHB/ty54nViYDkW1L8m7KuCv/bgUecMhqoh3.k31Fv18ZV7Ik4wXS', '2026-03-25 08:56:25.965695+00', '2026-03-25 08:56:25.842918+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "5a8e9c0b-545e-4b2b-8058-895831c9dec8", "email": "rasmus.willehard@gmail.comnbhg", "username": "moi", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('71c7e00c-fd0a-43e3-b97d-d1d809dc80a7', 'rasmus.willehard@gmail', '$2a$10$URcBkHzGNAUsXCFkibv8JOpoUdMrYMXT/vrDdBJKW.y14AtP6CRKy', '2026-03-25 13:28:49.410087+00', '2026-03-25 13:28:49.349748+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "71c7e00c-fd0a-43e3-b97d-d1d809dc80a7", "email": "rasmus.willehard@gmail", "username": "elite", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('bced39ae-ca8f-4120-a8eb-2b27e834e94b', 'demo@thewtracker.com', '$2a$10$SIbKWFyIoyA2yjoVC44Tm.rDqUNhMRjkCWGZofrdfWUQV.Bu4Ptla', '2026-03-25 13:33:13.053235+00', '2026-03-25 13:33:13.023748+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"username": "demo_user", "email_verified": true}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('1ed4be4e-91af-4319-a190-930bad490db9', 'hjugutgtntg@jjnjn.com', '$2a$10$g1OjHPCYVHUUuYHIXtEs3er2E1kEqL1mLwNHZkWgtelfL.LVV8zim', '2026-03-26 13:44:18.086186+00', '2026-03-26 13:44:17.993872+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "1ed4be4e-91af-4319-a190-930bad490db9", "email": "hjugutgtntg@jjnjn.com", "username": "www", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('b82fdf4e-9e62-4c9d-9f9d-93d58b4a259f', 'rasmius@jeidbi.flfi', '$2a$10$2sKahk60pRSjS6RMXvik7eIMmU5wUFCXwE0aeFWxh6DQ6zgBj4u4m', '2026-03-26 14:40:54.496506+00', '2026-03-26 14:40:54.463336+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "b82fdf4e-9e62-4c9d-9f9d-93d58b4a259f", "email": "rasmius@jeidbi.flfi", "username": "mmm", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('2d0d2152-1d78-4bff-bca0-8127e9c7b309', 'mdmddnbddn@ifif', '$2a$10$1t78QVIrgMzDNSncGAI8UuByMXvQklfEJg7YIaidguNw1Z.hI4oKC', '2026-03-26 14:48:25.082991+00', '2026-03-26 14:48:25.052852+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "2d0d2152-1d78-4bff-bca0-8127e9c7b309", "email": "mdmddnbddn@ifif", "username": "ppp", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('9293ab9b-a704-4a57-a5da-d9f5f24f6195', 'rasmus.willehard01@gmail.com', '$2a$10$VEfV4D8BZYgAKX9EUc6uUOJ.ulQMhVF55tAwDgrBmIjNA6eWGmMTu', '2026-03-26 15:51:55.039887+00', '2026-03-26 15:51:54.983747+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "9293ab9b-a704-4a57-a5da-d9f5f24f6195", "email": "rasmus.willehard01@gmail.com", "username": "aaaa", "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('c5ca7929-dfdf-40ff-953c-b13edda67a99', 'thewtracker@gmail.com', NULL, '2026-04-01 10:58:37.538157+00', '2026-04-01 10:58:37.47267+00', '{"provider": "apple", "providers": ["apple"]}'::jsonb, '{"iss": "https://appleid.apple.com", "sub": "000991.d2c40589fdd84f939812a983ffd1fa08.1058", "email": "thewtracker@gmail.com", "provider_id": "000991.d2c40589fdd84f939812a983ffd1fa08.1058", "custom_claims": {"auth_time": 1775055880}, "email_verified": true, "phone_verified": false}'::jsonb);
INSERT INTO auth.users (id, email, encrypted_password, email_confirmed_at, created_at, raw_app_meta_data, raw_user_meta_data) VALUES ('bf11735f-f450-497e-a231-75c9c09c54d8', 'emmi.innala@gmail.com', '$2a$10$0wuOum1YxntzLuXHY0glxeu08UGigZ96WT737LLohCXHYXsS3l.6e', '2026-04-02 16:43:41.552634+00', '2026-04-02 16:43:13.102417+00', '{"provider": "email", "providers": ["email"]}'::jsonb, '{"sub": "bf11735f-f450-497e-a231-75c9c09c54d8", "email": "emmi.innala@gmail.com", "username": "emister", "email_verified": true, "phone_verified": false}'::jsonb);


-- ============================================================
-- C2. auth.identities INSERT statements (email/Apple/Google identity links)
-- ============================================================
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('01a63c98-3dcd-4666-9be1-182d11c3e066', '01a63c98-3dcd-4666-9be1-182d11c3e066', '{"sub": "01a63c98-3dcd-4666-9be1-182d11c3e066", "email": "rasmus.willehard@gmail.com", "username": "willehard", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 09:46:52.41331+00', '2026-03-22 09:46:52.413362+00', '2026-03-22 09:46:52.413362+00', 'rasmus.willehard@gmail.com', 'e2160c0e-f374-40f0-aa12-ce3ebac8ef46');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('63752d9b-c1ce-498f-9994-d38c462c3c6b', '63752d9b-c1ce-498f-9994-d38c462c3c6b', '{"sub": "63752d9b-c1ce-498f-9994-d38c462c3c6b", "email": "rasmus.willehard@gmail.fi", "username": "moneymogger888", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 10:16:29.805338+00', '2026-03-22 10:16:29.805728+00', '2026-03-22 10:16:29.805728+00', 'rasmus.willehard@gmail.fi', 'f750e849-a72c-4fa7-9e9f-e1167c1ae003');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('9cbdb328-fbc4-4a00-b5f6-f093a5b77827', '9cbdb328-fbc4-4a00-b5f6-f093a5b77827', '{"sub": "9cbdb328-fbc4-4a00-b5f6-f093a5b77827", "email": "rasmus.willehard@gmail.co", "username": "kingmaxing", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 10:43:34.985823+00', '2026-03-22 10:43:34.985875+00', '2026-03-22 10:43:34.985875+00', 'rasmus.willehard@gmail.co', '7eb15cc3-e99b-4b0e-9d0c-f35a12d6a91e');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('006339bd-b83a-443a-ade5-2c2efc6d8efd', '006339bd-b83a-443a-ade5-2c2efc6d8efd', '{"sub": "006339bd-b83a-443a-ade5-2c2efc6d8efd", "email": "rosa.petterson1@gmail.com", "username": "rosa11", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 12:32:31.975034+00', '2026-03-22 12:32:31.975851+00', '2026-03-22 12:32:31.975851+00', 'rosa.petterson1@gmail.com', '2c835da2-3c75-4017-be77-b4b766358d1c');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('a46ff5cb-c0fe-4ceb-9743-7a68e7a48f6d', 'a46ff5cb-c0fe-4ceb-9743-7a68e7a48f6d', '{"sub": "a46ff5cb-c0fe-4ceb-9743-7a68e7a48f6d", "email": "moi@moi", "username": "elitesensei", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 14:00:42.86318+00', '2026-03-22 14:00:42.863235+00', '2026-03-22 14:00:42.863235+00', 'moi@moi', '6992a1e7-d77c-4b76-b512-69b0a3cebda9');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('d95f5868-c4f7-4403-ad85-f2a35797f03b', 'd95f5868-c4f7-4403-ad85-f2a35797f03b', '{"sub": "d95f5868-c4f7-4403-ad85-f2a35797f03b", "email": "mijjujj@njnnjn.com", "username": "wgroup", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 16:40:14.531015+00', '2026-03-22 16:40:14.531086+00', '2026-03-22 16:40:14.531086+00', 'mijjujj@njnnjn.com', '9200102f-45f5-41ef-bb53-038b17520c6c');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('779580c8-93a0-43d2-9022-49a57c0dadc5', '779580c8-93a0-43d2-9022-49a57c0dadc5', '{"sub": "779580c8-93a0-43d2-9022-49a57c0dadc5", "email": "moimoi@hmiu", "username": "sisu", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-22 17:11:18.318214+00', '2026-03-22 17:11:18.318262+00', '2026-03-22 17:11:18.318262+00', 'moimoi@hmiu', '4492797e-47a3-4eb4-9075-9d2be44f08ce');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('d7b989be-006f-4e21-aa39-e21642f56d8a', 'd7b989be-006f-4e21-aa39-e21642f56d8a', '{"sub": "d7b989be-006f-4e21-aa39-e21642f56d8a", "email": "iipo.kajalaa@gmail.com", "username": "relentlessrise", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-23 07:48:26.100763+00', '2026-03-23 07:48:26.100813+00', '2026-03-23 07:48:26.100813+00', 'iipo.kajalaa@gmail.com', '0c178b80-02e9-4c44-83fa-908d7d117adf');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('8a731910-5b1f-4acc-8f27-9d6915054c42', '8a731910-5b1f-4acc-8f27-9d6915054c42', '{"sub": "8a731910-5b1f-4acc-8f27-9d6915054c42", "email": "rasmus.willehard@gm", "username": "mogger", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-24 10:03:27.557423+00', '2026-03-24 10:03:27.558742+00', '2026-03-24 10:03:27.558742+00', 'rasmus.willehard@gm', '098f3035-bc67-415f-95fd-2074970e76b0');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('e7d58531-5700-4188-8551-6ef815017aa5', 'e7d58531-5700-4188-8551-6ef815017aa5', '{"sub": "e7d58531-5700-4188-8551-6ef815017aa5", "email": "mokok@loko", "username": "moimoi", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-24 14:46:33.691197+00', '2026-03-24 14:46:33.691244+00', '2026-03-24 14:46:33.691244+00', 'mokok@loko', '38fccc7d-6a5f-4cbc-9205-e2d9795ce63f');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('2e68a67b-a09b-4e40-bd21-58d1ee1f98c8', '2e68a67b-a09b-4e40-bd21-58d1ee1f98c8', '{"sub": "2e68a67b-a09b-4e40-bd21-58d1ee1f98c8", "email": "rasmus.willehard@gmail.commm", "username": "mogmaxing", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-25 07:10:46.401293+00', '2026-03-25 07:10:46.401352+00', '2026-03-25 07:10:46.401352+00', 'rasmus.willehard@gmail.commm', 'afde1ac9-5fd1-4d18-9cfe-522836b8c3f2');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('f57f89d5-f091-4c8f-a4cf-0702da7bb4c6', 'f57f89d5-f091-4c8f-a4cf-0702da7bb4c6', '{"sub": "f57f89d5-f091-4c8f-a4cf-0702da7bb4c6", "email": "rasmus.willehard@gmail.commmmm", "username": "mogging", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-25 07:13:38.344988+00', '2026-03-25 07:13:38.34539+00', '2026-03-25 07:13:38.34539+00', 'rasmus.willehard@gmail.commmmm', '50927eca-e77b-4bfc-a346-67e8a8b7a7cc');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('5a8e9c0b-545e-4b2b-8058-895831c9dec8', '5a8e9c0b-545e-4b2b-8058-895831c9dec8', '{"sub": "5a8e9c0b-545e-4b2b-8058-895831c9dec8", "email": "rasmus.willehard@gmail.comnbhg", "username": "moi", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-25 08:56:25.952096+00', '2026-03-25 08:56:25.952571+00', '2026-03-25 08:56:25.952571+00', 'rasmus.willehard@gmail.comnbhg', 'a45b3ef6-eb30-4f5c-b7a2-1006cdc745fc');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('71c7e00c-fd0a-43e3-b97d-d1d809dc80a7', '71c7e00c-fd0a-43e3-b97d-d1d809dc80a7', '{"sub": "71c7e00c-fd0a-43e3-b97d-d1d809dc80a7", "email": "rasmus.willehard@gmail", "username": "elite", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-25 13:28:49.395874+00', '2026-03-25 13:28:49.396592+00', '2026-03-25 13:28:49.396592+00', 'rasmus.willehard@gmail', '7c94f451-fedf-4a18-8ea3-eb69fbc18c84');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('bced39ae-ca8f-4120-a8eb-2b27e834e94b', 'bced39ae-ca8f-4120-a8eb-2b27e834e94b', '{"sub": "bced39ae-ca8f-4120-a8eb-2b27e834e94b", "email": "demo@thewtracker.com", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-25 13:33:13.047724+00', '2026-03-25 13:33:13.04778+00', '2026-03-25 13:33:13.04778+00', 'demo@thewtracker.com', '5db91aa4-f2d3-4793-8278-75233e36215f');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('1ed4be4e-91af-4319-a190-930bad490db9', '1ed4be4e-91af-4319-a190-930bad490db9', '{"sub": "1ed4be4e-91af-4319-a190-930bad490db9", "email": "hjugutgtntg@jjnjn.com", "username": "www", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-26 13:44:18.075032+00', '2026-03-26 13:44:18.075096+00', '2026-03-26 13:44:18.075096+00', 'hjugutgtntg@jjnjn.com', '3a9c0474-4dae-4f79-9408-082a23aa6ad5');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('b82fdf4e-9e62-4c9d-9f9d-93d58b4a259f', 'b82fdf4e-9e62-4c9d-9f9d-93d58b4a259f', '{"sub": "b82fdf4e-9e62-4c9d-9f9d-93d58b4a259f", "email": "rasmius@jeidbi.flfi", "username": "mmm", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-26 14:40:54.488476+00', '2026-03-26 14:40:54.488535+00', '2026-03-26 14:40:54.488535+00', 'rasmius@jeidbi.flfi', 'bb28cfc2-7221-4558-8cb4-10ee4877a12c');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('2d0d2152-1d78-4bff-bca0-8127e9c7b309', '2d0d2152-1d78-4bff-bca0-8127e9c7b309', '{"sub": "2d0d2152-1d78-4bff-bca0-8127e9c7b309", "email": "mdmddnbddn@ifif", "username": "ppp", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-26 14:48:25.075411+00', '2026-03-26 14:48:25.075463+00', '2026-03-26 14:48:25.075463+00', 'mdmddnbddn@ifif', '428ed3dd-5d5c-4c79-9547-803f49efba43');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('9293ab9b-a704-4a57-a5da-d9f5f24f6195', '9293ab9b-a704-4a57-a5da-d9f5f24f6195', '{"sub": "9293ab9b-a704-4a57-a5da-d9f5f24f6195", "email": "rasmus.willehard01@gmail.com", "username": "aaaa", "email_verified": false, "phone_verified": false}'::jsonb, 'email', '2026-03-26 15:51:55.032032+00', '2026-03-26 15:51:55.032101+00', '2026-03-26 15:51:55.032101+00', 'rasmus.willehard01@gmail.com', 'e4656468-3b4d-4dbf-8e4a-eb15973bb724');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('115441800927733729493', '01a63c98-3dcd-4666-9be1-182d11c3e066', '{"iss": "https://accounts.google.com", "sub": "115441800927733729493", "name": "Rasmus Petterson", "email": "rasmus.willehard@gmail.com", "picture": "https://lh3.googleusercontent.com/a/ACg8ocKNEy6Fxp-zLJkEoj8Rxp75ZsIBDN1AfoZ9lqizB5f9q8z6=s96-c", "full_name": "Rasmus Petterson", "avatar_url": "https://lh3.googleusercontent.com/a/ACg8ocKNEy6Fxp-zLJkEoj8Rxp75ZsIBDN1AfoZ9lqizB5f9q8z6=s96-c", "provider_id": "115441800927733729493", "email_verified": true, "phone_verified": false}'::jsonb, 'google', '2026-03-27 07:45:17.602904+00', '2026-03-27 07:45:17.60296+00', '2026-03-28 18:55:24.825643+00', 'rasmus.willehard@gmail.com', '5c0372b9-5888-498a-9686-78823cb93a42');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('000991.d2c40589fdd84f939812a983ffd1fa08.1058', 'c5ca7929-dfdf-40ff-953c-b13edda67a99', '{"iss": "https://appleid.apple.com", "sub": "000991.d2c40589fdd84f939812a983ffd1fa08.1058", "email": "thewtracker@gmail.com", "provider_id": "000991.d2c40589fdd84f939812a983ffd1fa08.1058", "custom_claims": {"auth_time": 1775055880}, "email_verified": true, "phone_verified": false}'::jsonb, 'apple', '2026-04-01 10:58:37.529745+00', '2026-04-01 10:58:37.530145+00', '2026-04-01 15:04:41.775487+00', 'thewtracker@gmail.com', 'fb48d410-db83-495b-bfe6-d2537110547f');
INSERT INTO auth.identities (provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at, email, id) VALUES ('bf11735f-f450-497e-a231-75c9c09c54d8', 'bf11735f-f450-497e-a231-75c9c09c54d8', '{"sub": "bf11735f-f450-497e-a231-75c9c09c54d8", "email": "emmi.innala@gmail.com", "username": "emister", "email_verified": true, "phone_verified": false}'::jsonb, 'email', '2026-04-02 16:43:13.187449+00', '2026-04-02 16:43:13.187506+00', '2026-04-02 16:43:13.187506+00', 'emmi.innala@gmail.com', 'b416c51c-218b-4fbc-b72c-c3f7e36f97b5');

-- ============================================================
-- D. Full public schema + data dump from pg_dump
-- ============================================================
--
-- PostgreSQL database dump
--

\restrict eCkJJ7ic6mjcibh3Ek498HXmxqC0IWUPl1uZStck9A4e8sJFFfAX0gW8hh7qzwu
...
\unrestrict eCkJJ7ic6mjcibh3Ek498HXmxqC0IWUPl1uZStck9A4e8sJFFfAX0gW8hh7qzwu
