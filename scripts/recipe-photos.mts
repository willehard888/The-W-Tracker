 ...
 *  - Encode to WebP. There is no image tooling in this repo, so the conversion
 *    ran through `npx sharp-cli` rather than adding a dependency: square at
 *    -q 72, thumb resized to 560 at -q 80. Quality 72 was picked over 82 after
 *    comparing them by eye — indistinguishable on screen, 831 KB instead of
 *    1135 KB across the set, which keeps faith with the pass that made the
 *    bundle 8 MB lighter.
...
/** square = list tile + detail hero (1000px); thumb = small tile (560px). Both WebP. */
...
for (const r of RECIPES) {
  ...
  for (const dir of SIZES) {
    if (!existsSync(resolve(ROOT, "src/assets/recipes", dir, `${r.id}.webp`))) {
      problems.push(`${r.id}: missing ${dir}/${r.id}.webp`);
    }
  }
}
