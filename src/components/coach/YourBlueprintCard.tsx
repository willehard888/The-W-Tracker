  ...
  return (
    <button
      ...
      className={cn(
        ...
        "p-5 active:scale-[0.99] transition-transform shadow-[0_18px_48px_-24px_hsl(42_78%_54%/0.4)]",
        className,
      )}
      aria-label="Edit your W Coach blueprint"
    >
      <div className="flex items-start gap-3">
        <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-gold to-[hsl(42_78%_42%)] flex items-center justify-center shrink-0 shadow-[0_0_14px_hsl(42_78%_54%/0.4)]">
      ...
      {hobbies.length > 0 && (
        <div className="mt-3 flex items-center gap-2 flex-wrap">
          <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground/70">
            <Heart size={10} className="text-gold/70" aria-hidden /> Joy
          </span>
          {hobbies.map((h) => (
            <span
              key={h}
              className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-card/80 border border-border/60 text-foreground/90"
            >
              {h}
            </span>
          ))}
