...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      ...
      // backdrop-filter as solid black, hiding sheet contents underneath.
      // Solid bg-black/72 keeps the depth without the render bug.
...
const sheetVariants = cva(
  "surface-glass fixed z-50 gap-4 p-6 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        ...
        <SheetPrimitive.Close
          className={cn(
            ...
            // Visual 32px, tap target expanded to Apple's 44pt minimum via an
            // invisible inset — pseudo-element takes the taps, no visual change.
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn("text-display-md text-foreground", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description ref={ref} className={cn("text-body text-fg-muted", className)} {...props} />
));
