-- 1. Update update_status_tier: respect legend_pinned + elevate apex subscribers to apex tier minimum
...
BEGIN
  -- Pinned legends and subscribers — short-circuit
  ...
  -- Apex subscribers (active or with credits) always display at apex minimum
  ...
  IF percentile >= 99.9 AND activity_days >= 30 AND user_streak >= 30 THEN
    new_tier := 'legend';
  ELSIF percentile >= 99 AND activity_days >= 30 AND user_streak >= 30 THEN
    new_tier := 'apex';
...
-- 2. Same updates for batch update_all_status_tiers
...
BEGIN
  ...
  -- Pinned legends — keep as legend, skip the rest of evaluation
  UPDATE profiles SET status_tier = 'legend' WHERE legend_pinned = true;

  -- Apex subscribers — always at apex minimum
  ...
  -- Earned tiers: only for non-pinned, non-subscriber users
  FOR r IN
  ...
  -- Recruits without rank score (still skip pinned and subscribers)
  UPDATE profiles
    SET status_tier = 'recruit'
    WHERE rank_score = 0
      AND legend_pinned = false
...
-- 3. Pin Legend status to Willehard and relentlessrise
UPDATE profiles
...
-- 4. Recompute everyone (apex subscribers will jump to apex, pinned legends stay legend)
