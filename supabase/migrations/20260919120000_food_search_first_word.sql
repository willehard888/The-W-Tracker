-- Food search: the word itself before its compounds.
-- Follows 20260919110000. Two gaps it left, nothing else changes (body is that
-- definition):
--   1. Every name that starts with the query scored 0.9 and the tie went to the
--      shortest, so "maito" listed "Maitosuklaa" above "Maito, kevyt", "kana"
--      listed "Kananmuna" above "Kana, …" and "egg" listed "Eggnog" above
--      "Egg, whole". A name whose FIRST WORD is the query (plural in -s / -es
--      either way round) now scores 0.97: above the other prefixes, and with the
--      generic edge (+0.04) above a branded product named exactly the query.
--   2. The 200-row candidate cut ordered by the fuzzy score alone, where every row
--      holding the exact word ties at 1.0 and the shortest win: with 250 rows
--      named "Soup …, chicken", "Chicken, broilers or fryers, breast …" never
--      reached the ranking. Names that start with the query are cut last.
          ...
          SELECT f.id,
                 row_number() OVER (
                   ...
                   ORDER BY (v_country IS NOT NULL AND f.country = v_country) DESC,
                            -- search_text opens with the name: keep what the ranking puts first
                            (f.search_text LIKE v_q || '%') DESC,
  ...
  food_rows AS (
    ...
    CROSS JOIN LATERAL (
      SELECT (CASE
                ...
                -- The word itself before its compounds: "Maito, kevyt" before
                -- "Maitosuklaa", "Bananas, raw" for "banana".
                -- Plural either way round: "banana" finds "Bananas", "eggs" finds "Egg".
                WHEN substring(public.f_unaccent(lower(f.name)) from '^[[:alnum:]]+')
                     IN (v_q, v_q || 's', v_q || 'es', regexp_replace(v_q, 's$', ''), regexp_replace(v_q, 'es$', '')) THEN 0.97
